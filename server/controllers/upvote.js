...
const voteQuestion = (req, res) => {
  ...
  if (question) {
    ...
    if (!(question.upvotedBy.find(cuserId => compare(userId, cuserId))) && !(question.downvotedBy.find(user_id => compare(user_id, userId)))) {
      question.upvotes += 1;
      question.upvotedBy.push(userId);
    }

    // if none of the above then check if the user have downvoted and shift the vote to upvote
    else if (question.downvotedBy.find(userid => compare(userid, userId))) {
